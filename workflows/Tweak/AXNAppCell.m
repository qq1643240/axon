...
-(id)initWithFrame:(CGRect)frame {
    ...
    prefs = [[NSMutableDictionary alloc] initWithContentsOfFile:getPrefsPath()];
    self.badgesEnabled = prefs[@"BadgesEnabled"] != nil ? [prefs[@"BadgesEnabled"] boolValue] : true;

    // Long press gesture for menu (existing)
    ...
    // Swipe up gesture - clear single app notifications
    UISwipeGestureRecognizer *swipeUp = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeUp:)];
    swipeUp.direction = UISwipeGestureRecognizerDirectionUp;
    [self addGestureRecognizer:swipeUp];

    // Swipe down gesture - clear all notifications
...
-(void)handleSwipeDown:(UISwipeGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateEnded) {
        if (hapticFeedback) AudioServicesPlaySystemSound(1520);

        // Animate all cells swipe down
        AXNView *parentView = (AXNView *)self.superview;
        if ([parentView isKindOfClass:[UICollectionView class]]) {
...
NSString* getPrefsPath() {
    ...
    return jbroot(@"/var/mobile/Library/Preferences/me.nepeta.axon.plist");
#else
    return @"/var/mobile/Library/Preferences/me.nepeta.axon.plist";
#endif
}
